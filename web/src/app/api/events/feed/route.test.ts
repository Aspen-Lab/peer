 ...
 *
 * **What it pins.** Before 1-05, an unauthenticated `POST` here produced **seven**
 * outgoing searches on the operator's Tavily key — the largest single leak round-1
 * A found. The number that has to stay at
...
function request(body: Record<string, unknown>): NextRequest {
  return new NextRequest("http://localhost/api/events/feed", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
describe("POST /api/events/feed — the operator's search key", () => {
  it("spends nothing for an anonymous caller", async () => {
    // A's round-1 measurement: 7 outgoing searches, from a request with no
  ...
  it("spends nothing for a signed-in free user with no key", async () => {
    ...
    // R-POOL-3 — events still respond from the free structured sources, so
    // this is a 200 and not a 401.
    expect(response.status).toBe(200);
    expect(requestsCarrying(OPERATOR_SENTINEL)).toEqual([]);
  });
